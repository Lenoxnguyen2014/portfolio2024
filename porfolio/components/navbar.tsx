      ...
      <nav className='bg-black border-gray-200 '>
        ...
        {links.map(({ href, label }) => (
              <li key={href} className="text-sm md:text-xl font-bold text-white pt-2 max-sm:text-lg">
                <a className={`inline-flex px-3 py-1.5 white hover:text-cyan-500 [&.active]:underline decoration-cyan-400 [&.active]:text-white ${pathname === `${href}` ? 'active' : ''}`} href={href}>{label}</a>
              </li>
        ))}
